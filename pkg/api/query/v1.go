...
func (qapi *QueryAPI) query(r *http.Request) (any, []error, *api.ApiError, func()) {
	...
	// Add wire bytes trace attribute if we have an active span
	if span := opentracing.SpanFromContext(ctx); span != nil {
		wireBytes := calculateWireBytes(responseData)
...
// calculateWireBytes estimates the wire protocol size by marshaling the result to JSON.
func calculateWireBytes(data any) int64 {
	responseBytes, err := json.Marshal(data)
	if err != nil {
		// If marshaling fails, return 0
		return 0
	}
